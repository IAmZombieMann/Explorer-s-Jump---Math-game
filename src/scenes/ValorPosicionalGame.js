...
export class ValorPosicionalGame extends Phaser.Scene {
    ...
    create() {
        ...
        // 1. FONDO (Limpio y moderno, sin bombones)
        this.add.rectangle(width/2, height/2, width, height, 0x1a1a2e).setDepth(0);
        
        // Detalles sutiles de fondo (estrellitas abstractas) para no dejarlo tan plano
        ...
        // 2. BOTÓN VOLVER (Estilo pill-shape más elegante)
        const btnVolver = this.add.container(80, 50).setDepth(1000);
        const bgVolver = this.add.graphics({ fillStyle: { color: 0xe74c3c } });
        bgVolver.fillRoundedRect(-55, -20, 110, 40, 20); // Bordes totalmente redondos
        ...
        // 3. ZONA DE DEPÓSITO (Caja receptora más atractiva y "tecnológica")
        ...
        // 4. TEXTOS (Objetivo Fijo + Feedback Dinámico)
        this.numeroObjetivo = Phaser.Math.Between(10, 150);
        const tituloY = 120;
        
        // Cartel del objetivo (Fijo)
        ...
        bgTitulo.lineStyle(4, 0xf1c40f, 1); // Borde amarillo oro
        ...
        // Texto de Feedback (Dinámico, debajo del objetivo)
        ...
        // 5. GENERADORES (Escala grande para touch)
        ...
        // 7. BOTÓN COMPROBAR (Pegado a la caja)
        this.createCheckButton(this.zonaX, this.limitesCaja.yMax + 35);
    }

    crearGenerador(x, y, tipo, etiqueta) {
        // Etiqueta con fondo tipo "píldora" para que resalte más y se vea limpio
    ...
    crearCopiaEnCaja(tipo, x, y) {
        ...
        // Feedback Visual (SIN BORRAR EL OBJETIVO)
    ...
    createCheckButton(x, y) {
        ...
        bg.fillRoundedRect(-140, -25, 280, 50, 15); // Botón de 280px para igualar el ancho de la caja
        ...
        bg.on('pointerup', () => {
            ...
            if (total === this.numeroObjetivo) {
                ...
                // ACTUALIZAMOS EL FEEDBACK PERO EL OBJETIVO QUEDA INTACTO
                this.uiFeedback.setText(`¡Llevas ${total}! Faltan piezas.`);
                this.uiFeedback.setColor('#e74c3c');
                
                this.cameras.main.shake(200, 0.01);
                
                // Regresamos el color del feedback a normal después de 2 segundos
